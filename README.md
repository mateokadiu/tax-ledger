...
- `tax_ledger_entries` — append-only. One row per emitted `LedgerEntry` (split rows, refund/capture/revision deltas).
...
| `VoidEvent` + `AddressChangeEvent` | Specialized reconcile events. Address-change works today as a `revise()` with new jurisdictions appearing — explicit event surface still pending. |
