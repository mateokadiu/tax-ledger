...
[![Status](https://img.shields.io/badge/status-v1.1-brightgreen)](#v11-scope-shipped)
[![Tests](https://img.shields.io/badge/tests-147-success)](#invariants-the-test-suite-proves)
...
MIT, public from day one. v1.1 ships `@tax-ledger/core` (the pure splitter + reconciler — `split` / `refund` / `partialCapture` / `revise` / `reconcile`) plus engine adapters for Avalara, TaxJar, Stripe Tax, and Fonoa, an opinionated Drizzle ORM adapter for persistence, multi-currency support with proper ISO-4217 minor-unit handling, deterministic (event-sourced-replay-friendly) verbs, and a Stripe `create_reversal` bridge. NestJS and CLI sugar are still on the roadmap.
...
pnpm add @tax-ledger/avalara @tax-ledger/taxjar @tax-ledger/stripe-tax @tax-ledger/fonoa
# + persistence
...
147 tests across the workspace (including ~21 property tests in `@tax-ledger/core` at 500-1000 runs each; plus adapter fixtures and a SQLite smoke suite). Each commit on `main` keeps the suite green.

## v1.1 scope (shipped)

| Module | Status |
|---|---|
| `@tax-ledger/core` — `split()`, `refund()` (amount + quantity), `partialCapture()`, `revise()`, `reconcile()` | shipped |
| Decimal.js + largest-remainder allocator with replay-stable tiebreak | shipped |
| Zod-validated input boundary (`TaxInput`) | shipped |
| Property-based test suite (`fast-check`) | shipped |
| Multi-currency with ISO-4217 minor-unit helpers (JPY=0, KWD=3, …); mixed-currency reconcile refused | shipped |
| Deterministic verbs — `{ now?, generateId? }` for snapshot tests / event-sourced replay | shipped |
| `'vat'` tax type + `taxCode` / `taxBehavior` / `engineTaxType` end-to-end | shipped |
| Reporting helpers — `toComponentTotals()` and `rollupBy([...])` | shipped |
| `@tax-ledger/avalara` — `CreateTransactionModel` → `TaxInput` adapter (VAT-aware, Bottle deposits) | shipped |
| `@tax-ledger/taxjar` — `taxes` response → `TaxInput` adapter | shipped |
| `@tax-ledger/stripe-tax` — `Tax.Calculation` → `TaxInput` adapter + `toStripeReversal()` / `reversalTotals()` bridge | shipped |
| `@tax-ledger/fonoa` — Fonoa Tax calculation → `TaxInput` (EU VAT / GST, inclusive pricing, reverse-charge) | shipped |
| `@tax-ledger/drizzle` — PG + SQLite schema (incl. `tax_code` / `tax_behavior` / `engine_tax_type` / `quantity` / `country` / `region` / `name`), `persistLedger`, `applyDelta`, migration generator | shipped |
| `examples/basic` — runnable Avalara → ledger → refund → reconcile demo | shipped |
...
pnpm test       # vitest run — 147 tests across 6 packages
```
